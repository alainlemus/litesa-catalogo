<div>
    ...
    @section('title', mb_strtoupper($post->title, 'UTF-8') . ' - Grupo Litesa')
    @section('meta_description', $post->excerpt)
    @section('og_title', mb_strtoupper($post->title, 'UTF-8') . ' - Grupo Litesa')
    @section('og_description', $post->excerpt)
    @section('og_image', Storage::disk('public')->url($post->image))

    @section('twitter_card', 'summary_large_image')
    @section('twitter_title', mb_strtoupper($post->title, 'UTF-8') . ' - Grupo Litesa')
    ...
    <div class="flex justify-center w-full px-4 py-10 mx-auto transition-colors duration-300 dark:bg-gray-700">
        ...
        <div class="max-w-3xl">
            ...
            <div class="flex my-6 space-x-4">
                ...
                <!-- Twitter/X -->
                <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->fullUrl()) }}&text={{ urlencode('Mira este artículo: ' . $post->title) }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-black hover:bg-gray-800 text-white" title="Compartir en X (Twitter)">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" class="w-5 h-5">
                ...
                <!-- LinkedIn -->
                <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode(request()->fullUrl()) }}&title={{ urlencode($post->title) }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-blue-800 hover:bg-blue-900 text-white" title="Compartir en LinkedIn">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" class="w-5 h-5">
            ...
            @if ($post->image)
                <img
                    ...
                    alt="{{ mb_strtoupper($post->title, 'UTF-8') }}"
            ...
            <div class="flex lg:justify-between flex-col lg:flex-row mb-4">
                @php
                    $fechaFormateada = Carbon::parse($post->updated_at)
                        ->translatedFormat('d \d\e\l \m\és F \d\e\l Y');
                    $fechaFormateada = str_replace(['É', '́'], ['E', ''], $fechaFormateada);
                @endphp
                <span class="mb-4 text-sm text-gray-400 dark:text-gray-500">{{ mb_strtoupper("Categoría: ".$post->category, 'UTF-8') }}</span>
                <time datetime="{{ $post->updated_at->toIso8601String() }}" class="mb-4 text-sm text-gray-400 dark:text-gray-500">{{ mb_strtoupper($fechaFormateada, 'UTF-8') }}</time>
            </div>
