                    ...
                    <div class="flex flex-col items-start justify-between">
                        <div class="w-full">
                            @if ($product->photos->count() > 0)
                                <img src="{{ env("APP_ENV", "local") ? asset('storage/' . $product->photos->first()->path) : storage::disk('s3')->url($product->photos->first()->path) }}" alt="{{ $product->name }}" class="object-contain w-full h-48 rounded">
                            @else
